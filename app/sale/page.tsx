    ...
    <div className="min-h-screen pt-20 relative">
      ...
      <section className="relative h-[50vh] w-full overflow-hidden border-b-8 border-primary bg-primary-dark">
        ...
        <div className="relative z-20 container mx-auto h-full flex flex-col justify-center px-4 md:px-6">
          <div className="brutalist-container bg-white max-w-2xl transform -rotate-2">
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        ...
        {isFilterOpen && (
          <div className="brutalist-container mb-8 transform rotate-1">
        ...
        {/* Call to Action */}
        <div className="mt-16 brutalist-container bg-accent text-white transform -rotate-1">
          <CircleDoodle className="absolute top-5 right-5 text-white opacity-30" />
          <h2 className="text-3xl font-bold mb-4 uppercase">Don't Miss Out!</h2>
          <p className="mb-6 uppercase">Sale ends soon. Shop now before your favorite items sell out!</p>
          <div className="flex justify-center">
            <AnimatedButton variant="white" animation="pulse" size="lg" iconPosition="right">
              Shop All Sale Items
            </AnimatedButton>
          </div>
        </div>
  ...
  return (
    <button
      ...
      className={`
        ...
        transform ${active ? "rotate-2" : "-rotate-1"} hover:rotate-0
        ${
          active
...
function CategoryButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`
        relative px-4 py-2 text-sm font-bold uppercase transition-all hover-glitch
        transform ${active ? "rotate-2" : "-rotate-1"} hover:rotate-0
        ${
          active
  ...
  return (
    <div className="brutalist-card transform-card group">
      <div className="relative h-[300px]">
        ...
        {/* Quick actions */}
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t-4 border-accent p-2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
          <button className="p-2 border-2 border-accent hover-glitch transform hover:rotate-12">
            <Heart className="h-5 w-5" />
          </button>
          <button className="p-2 bg-accent text-white border-2 border-accent hover:bg-accent-hover transform hover:-rotate-3">
      ...
      <div className="p-4 border-t-4 border-accent">
        ...
        <div className="flex justify-between items-center">
          <div>
            <span className="font-bold text-accent transform rotate-3 inline-block">
              ${product.salePrice?.toFixed(2)}
            </span>
            <span className="ml-2 text-gray-500 line-through text-sm">${product.price.toFixed(2)}</span>
          </div>
          <div className="bg-accent text-white text-xs font-bold px-2 py-1 flex items-center transform -rotate-3">
  ...
  return (
    <div className="brutalist-card transform-card group">
      <div className="relative h-[400px]">
        ...
        {/* Quick actions */}
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t-4 border-primary p-4 flex justify-between">
          <div>
            <div className="text-lg font-bold uppercase">{product.name}</div>
            <div className="flex items-center mt-1">
              <span className="font-bold text-accent text-xl transform rotate-3 inline-block">
                ${product.salePrice?.toFixed(2)}
              </span>
